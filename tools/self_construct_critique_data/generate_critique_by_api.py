...
def process_chunk(
    ...
    for item in tqdm(chunk_data, desc=f"Process {process_id}"):
        ...
        try:
            ...
            item["cost"] = (
                completion.usage.completion_tokens * 10 / 1e6
                + completion.usage.prompt_tokens * 2.5 / 1e6
            )
            results.append(item)
...
def process_large_dataset(
    model_name: str,
    # raw data (question + answer), for LLM to critique
    input_path: str,
    output_path: str,
    prompt_func: Callable,
    num_processes: int = 1,
    ...
    if input_path in ["WebInstruct-CFT-50K"]:
        ...
        # contains math question and model generated answer
    ...
    # split each chunk into processes
    processes = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process large dataset with multiple processes"
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        "--input_path",
        type=str,
        default="WebInstruct-CFT-50K",
        help="Path to the input dataset file (default: WebInstruct-CFT-50K)",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default="critique_added_output_data.json",
    ...
    process_large_dataset(
        ...
        # change the prompt template here
